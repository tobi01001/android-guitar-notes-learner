...
class LowEDetectionTest {
    ...
    fun `test frequency detection accuracy across full guitar range`() {
        ...
        println(
            "${String.format(
                ...
                "%8s",
                "Expected",
            )} ${String.format("%8s", "Detected")} ${String.format("%8s", "Error")} ${String.format("%8s", "Cents")}",
        )
        println("-".repeat(80))
            ...
            if (result != null) {
                ...
                println(
                    "${String.format(
                        ...
                        "%8.2f",
                        frequency,
                    )} ${String.format("%8.2f", result.frequency)} ${String.format("%+8.2f", error)} ${String.format("%+8.1f", cents)}",
                )
                ...
                assert(abs(cents) < 200.0) {
                    // 200 cents = 2 semitones (generous tolerance)
                    "$note: Cents error too large: $cents cents"
                }
            } else {
                println(
                    "${String.format("%-6s", note)} ${String.format("%8.2f", frequency)} ${String.format("%8s", "FAILED")} - no detection",
                )
    ...
    private fun calculateSemitoneError(
        detected: Double,
        expected: Double,
    ): Double = 12 * kotlin.math.log2(detected / expected)
