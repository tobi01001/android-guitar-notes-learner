...
class PitchDetectorTest {
    ...
    fun `detectPitch detects A4 frequency from synthesized sine wave`() {
        ...
        detected?.let {
            assertTrue("Detected frequency should be close to 440Hz", 
                it in 420.0..460.0)
        }
    }
    ...
    fun `detectPitch detects E2 frequency from synthesized sine wave`() {
        ...
        if (detected != null) {
            assertTrue("Detected frequency should be in valid range", 
    ...
    fun `detectPitch handles high frequency input`() {
        ...
        if (detected != null) {
            assertTrue("Any detected frequency should be in valid range",
                detected in 60.0..1500.0)
        }
    }
     ...
     */
    private fun generateSineWave(frequency: Double, sampleRate: Int, samples: Int): ShortArray {
