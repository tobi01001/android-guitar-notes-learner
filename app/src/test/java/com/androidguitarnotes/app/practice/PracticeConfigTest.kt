...
class PracticeConfigTest {
    ...
    fun `default config has WHOLE_NOTES mode`() {
        ...
        assertEquals("Default note mode should be WHOLE_NOTES", 
            NoteMode.WHOLE_NOTES, config.noteMode)
    }
    ...
    fun `default config has TIME duration type`() {
        ...
        assertEquals("Default duration type should be TIME", 
            DurationType.TIME, config.durationType)
    }
